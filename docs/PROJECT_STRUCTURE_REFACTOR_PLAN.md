...
执行结果：第 1 至 6 项已经完成；第 7 项没有执行。`var/snapshots/` 中的三个文件已失去生成调用方，但仍作为历史实物保留；根数据库和兼容 URL 路由也保持不变。
...
- 静态生成和旧 JSON 读写链路已经在阶段 5 退役；旧 JSON 归档、根数据库副本和现存快照文件仍保留，只有取得各自确认后才能删除。
...
4. **清退确认（部分完成，2026-07-17）**：用户明确同意进入阶段 5，已授权删除旧静态生成链路、旧 JSON 读写与无调用方兼容包装。本次授权不解释为删除根数据库副本、`data/archive/data.json`、现存快照文件或兼容 URL 路由；这些对象仍需分别再次确认。
